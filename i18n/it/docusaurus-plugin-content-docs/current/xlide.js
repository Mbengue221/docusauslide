var xLide = class xLide {
    ...
    processCtrlBar() {
        if (typeof document !== typeof undefined) {
            ...
            if (this.hasCtrlBar()) {
                let previews = null, leftCtrl = null, rightCtrl = null;
    ...
    xlide() {
        ...
        this.target.removeEventListener('mouseenter', e => {
            this.is_playing = false;
        });
        this.target.removeEventListener('mouseleave', e => {
            this.is_playing = true;
            this.move();
        });
    ...
    reset_left_arrow_event() {
        this.slider.querySelectorAll('.before').forEach(before => {
            before.addEventListener('click', e => this.prevSlide());
        });
        return this;
    }
    reset_right_arrow_event() {
        this.slider.querySelectorAll('.after').forEach(before => {
    ...
    reset_preview_events(preview, idx) {
        preview.removeEventListener('click', e => {
            this.slideTo(idx);
        });
    ...
    reset_previews_events() {
        this.slider.querySelectorAll('.preview').forEach((elem, idx) => {
    ...
    slidesOut() {
        this.slider.querySelectorAll('.xlide-item').forEach(slide => {
    ...
    slideIn(idx) {
        this.slider.querySelectorAll('.xlide-item').forEach((slide, i) => {
            if (i == idx) {
                slide.classList.remove('out');
                slide.classList.add('in');
            }
    ...
    enablepreview(idx) {
        this.slider.querySelectorAll('.preview').forEach((elem, x) => {
            if (x == idx) {
...
var xlide = function xlide(target = document.createElement('section'), options) {
    if (typeof target === 'string') {
        target = document.querySelector(target);
    }
